...
--      below. B.4-B.6 always collapse to a B.4 restart regardless of
--      exactly which of the three failed, because raw email content
--      is never persisted outside CRM, so there is nothing to resume
--      mid-way through that block. B.7, B.8, and B.9 each get their
--      own precise resume point because their prerequisite outputs
...
(
    ...
    -- PRIOR attempt's last_completed_stage. NULL (no prior progress)
    -- and B1/B2 both collapse to B4 -- nothing durable exists yet to
    -- resume mid-way through data pull / parse / agent invocation.
